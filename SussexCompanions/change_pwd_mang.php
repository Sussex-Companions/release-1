...
$conn=new mysqli('localhost','root','','sc_club') or die(mysqli_error());
 if(count($_POST)>0){
  ...
  if (!$result) {
   ...
   if($sql)
        {
        echo "<script>alert('Password Changed Sucessfully'); window.location='Employee.php'</script>";
        }
       else
        {
       echo "<script>alert('Your new and Retype Password is not match'); window.location='change_pwd_mang.php'</script>";
       }    
 }
 ?> 
